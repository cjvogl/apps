      ...
      do 105 k = 1,nfil
       ...
       do 105 j = 1,ncol
        ...
        do 105 i=1,nxl
          ...
          if (r .lt. 0.3d0) then
c                # circular piston
                 val(1,i,j,k) = val(1,2*nxl+1-i,j,k)
                 val(2,i,j,k) = 2.d0*s - val(2,2*nxl+1-i,j,k)
                 val(3,i,j,k) = val(3,2*nxl+1-i,j,k)
                 val(4,i,j,k) = val(4,2*nxl+1-i,j,k)
c                write(6,*) '+++ t,r,s = ',time,r,s
            else if (r .lt. 0.6d0) then
c                # solid wall baffle around piston
                 val(1,i,j,k) = val(1,2*nxl+1-i,j,k)
                 val(2,i,j,k) = -val(2,2*nxl+1-i,j,k)
                 val(3,i,j,k) = val(3,2*nxl+1-i,j,k)
                 val(4,i,j,k) = val(4,2*nxl+1-i,j,k)
            else
c                # extrapolation (free space outflow)
                 do m=1,meqn
